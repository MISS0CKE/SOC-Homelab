...
output "splunk_url" {
  description = "Splunk Web URL on the private network (HTTP for current lab setup)."
  value       = "http://${azurerm_network_interface.splunk_nic.private_ip_address}:8000"
}

output "management_access_method" {
  description = "Management access path for lab VMs."
  value       = "Azure Bastion Standard with native client tunneling"
}
